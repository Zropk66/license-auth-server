...
This system features separate **Admin Management Portal** and **User Self-Service Portal**, along with a client verification and heartbeat API protected by envelope encryption (RSA-OAEP + AES-256-GCM) and Ed25519 response signatures — a man-in-the-middle can neither read nor tamper with any business data.
...
*   **Envelope-Encrypted Client Communication (v2 protocol)**: Each request generates an ephemeral session key, wrapped with RSA-OAEP-SHA256 and sent alongside an AES-256-GCM encrypted payload; responses are encrypted with the same key and carry an Ed25519 signature (sign-then-encrypt). A man-in-the-middle only sees random ciphertext; tampering with any byte breaks decryption. Undecryptable requests receive opaque random responses without revealing the failure reason.
...
*   **Security & Verification**: JWT, RSA-OAEP + AES-256-GCM envelope encryption, Ed25519 response signatures, Cloudflare Turnstile
...
3.  **Setup Environment Variables**:
    ...
    # Client communication key pairs (auto-generated by `npm run generate-keys`)
    # Ed25519: signing verify/heartbeat responses; RSA-2048: decrypting client request envelopes
...
1.  **Strong Secrets**: Ensure `JWT_SECRET` in `.env` is sufficiently complex (at least 16 characters). The `AUTH_PRIVATE_KEY` / `RSA_PRIVATE_KEY` private keys must remain server-side only — clients embed only public keys.
2.  **Key Rotation**: After re-running `npm run generate-keys --force`, update the public keys embedded in clients (`test-client/config.json` or `clients/cpp/update-keys.js`), otherwise clients cannot decrypt responses.
3.  **HTTPS Deployment**: The verification API transmits license keys and hardware IDs. Always configure HTTPS for transport-layer encryption.
4.  **Setup Token**: For production deployments, set `SETUP_TOKEN` to prevent unauthorized Owner account creation.
5.  **Turnstile**: Configure Cloudflare Turnstile to mitigate automated attacks.
